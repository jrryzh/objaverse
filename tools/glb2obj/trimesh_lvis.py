...
if __name__ == '__main__':
    start_index, end_index = 3000, 10000
    ...
    # 排序
    lvis_annotations_list = sorted(lvis_annotations_list)[start_index:end_index]
    ...
    for index, glb_id in enumerate(lvis_annotations_list):
        try:
            ...
            print(f'Converting {index+1}/{end_index-start_index}: Converted {glb_id} to OBJ format.')
        except:
            failed_index_list.append(index)
            print(f'Converting {index+1}/{end_index-start_index}: Failed to convert {glb_id} to OBJ format.')
            
    # 保存失败的索引
    with open(os.path.join(output_folder, f'failed_index_list_{start_index}_{end_index}.txt'), 'w') as f:
