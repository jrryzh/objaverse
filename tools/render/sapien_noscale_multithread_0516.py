...
def render_model(urdf_path, obj_path, objaverse_id, instance_path):
    
    from datetime import datetime

    current_time = datetime.now()
    print(f"{current_time} Start rendering {objaverse_id}")
    ...
    try:
        ...
        return urdf_path, "success"
    except Exception as e:
        return urdf_path, f"failed: {e}"
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for urdf_path in category2file[dir]:
            ...
            instance_path = os.path.join("/home/add_disk_e/objaverse_lvis_trimesh_normalized_objs_output/", dir, objaverse_id)
            # if not os.path.exists(instance_path):
            #     os.makedirs(instance_path)
            # else:
            #     print(f"{objaverse_id} already exists, skip")
            #     continue
            file_count = len([name for name in os.listdir(instance_path) if os.path.isfile(name)])
            if file_count > 10:
                print(f"{objaverse_id} already has {file_count} files, skip")
                continue
            futures.append(executor.submit(render_model, urdf_path, obj_path, objaverse_id, instance_path))

        for future in as_completed(futures, timeout=timeout):
            try:
                urdf_path, status = future.result()
                if status == "success":
                    successful_urdf_lst.append(urdf_path)
                else:
                    failed_urdf_lst.append((urdf_path, status))
            except Exception as e:
                failed_urdf_lst.append((urdf_path, f"Exception: {e}"))
