...
# 计算逆平移和缩放变换矩阵
def apply_inverse_translation_and_scale(instance_pose, centroid, scale):
    # 生成平移矩阵
    inverse_translation_matrix = np.eye(4)
    inverse_translation_matrix[:3, 3] = centroid * scale  # 反向平移
    
    # 生成缩放矩阵
    inverse_scale_matrix = np.eye(4)
    inverse_scale_matrix[:3, :3] /= scale  # 反向缩放
    
    # 先应用反向平移变换，再应用反向缩放变换
    restored_pose = np.dot(inverse_scale_matrix, np.dot(inverse_translation_matrix, instance_pose))
    return restored_pose

# 计算新pipeline的pose矩阵
def get_new_pose(cam_pose, scale, centroid):
    new_pose = 
...
def main():
    objaverse_dir = "/home/add_disk_e/objaverse_lvis_trimesh_objs/"
    category2file = {}
    ...
    for dir in category2file:
        dir = "water_bottle"
        for urdf_path in category2file[dir][:1]:
            ...
            try:
                ...
                instance_path = os.path.join("/home/fudan248/zhangjinyu/tmp", dir, objaverse_id) # debug only
                ...
                loader.scale = 1.0 / max(X_scale, Y_scale, Z_scale)
                ...
                current_pose = asset.get_pose()
                new_position = current_pose.p - centroid * loader.scale 
                asset.set_pose(sapien.Pose(new_position, current_pose.q))
                
                print("Current pose: ", current_pose)
                print("New position: ", new_position)
                print("scale: ", loader.scale)
                print("New pose: ", asset.get_pose())
                ...
                for i, cam_pos in enumerate(camera_poses):
                    ...
                    # change
                    instance_pose = np.linalg.inv(get_new_pose(new_cam_pos, scale, centroid))
                    import ipdb; ipdb.set_trace()
